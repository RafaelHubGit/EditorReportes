...
export const DocumentTitle = ({ 
  ...
  <Space size={0} align="start" style={{ display: 'flex', flexDirection: 'column' }}>
    {isEditing && !readOnly ? (
      ...
      <Space>
        <Title level={3} style={{ margin: 0, cursor: readOnly ? 'default' : 'pointer' }} onClick={readOnly ? undefined : onEditStart}>
          {name}
        </Title>
        {!readOnly && <Button type="text" icon={<EditOutlined />} size="small" onClick={onEditStart} />}
      </Space>
    )}
